...
- **Figma интеграция**: Работа с Figma файлами через MCP сервер (опционально)
...
5. Установите Node.js (требуется для Figma MCP сервера, опционально):
```bash
# Проверьте, установлен ли Node.js:
node --version
# Если не установлен, установите с https://nodejs.org/
```
...
### Figma инструменты (опционально)
- `figma_get_file` - получение информации о Figma файле, фрейме или группе по ссылке
- `figma_list_tools` - список доступных инструментов Figma MCP сервера
...
**Figma интеграция:** Для работы с Figma требуется:
1. Установленный Node.js
2. API ключ Figma (получите на https://www.figma.com/developers/api#access-tokens)
3. Указать `figma.api_key` в `local.properties`
...
# Figma Configuration (опционально)
figma.api_key=your_figma_api_key_here
